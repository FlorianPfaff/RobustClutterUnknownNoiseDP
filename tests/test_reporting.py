...
def test_compare_to_reference_paired_uses_matched_seed_deltas():
    results = [
        _result("dp", seed=0, false_tracks=2, gospa_false_cost=4.0, missed_targets=1),
        _result("oracle", seed=0, false_tracks=1, gospa_false_cost=2.0, missed_targets=1),
        _result("dp", seed=1, false_tracks=4, gospa_false_cost=8.0, missed_targets=2),
        _result("oracle", seed=1, false_tracks=1, gospa_false_cost=2.0, missed_targets=0),
        # No reference for seed 2, so this row should be ignored by the paired comparison.
        _result("dp", seed=2, false_tracks=10, gospa_false_cost=20.0, missed_targets=3),
    ]

    comparisons = compare_to_reference_paired(results, reference_method="oracle")

    assert len(comparisons) == 1
    comparison = comparisons[0]
    assert comparison.scenario == "hotspot"
    assert comparison.method == "dp"
    assert comparison.num_pairs == 2
    assert comparison.mean_delta_false_tracks == 2.0
    assert comparison.mean_delta_gospa_false_cost == 4.0
    assert comparison.mean_delta_missed_targets == 1.0
    assert comparison.se_delta_false_tracks > 0.0
...
def test_format_method_csv_helpers_include_headers_and_rows():
    ...
    comparisons = compare_to_reference(aggregate_method_results(comparison_results))
    paired_comparisons = compare_to_reference_paired(comparison_results)
    ...
    assert paired_comparison_csv.startswith("scenario,method,reference_method,num_pairs")
    assert "se_delta_false_tracks" in paired_comparison_csv
